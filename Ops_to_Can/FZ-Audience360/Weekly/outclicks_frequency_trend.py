...
value_df = pd.read_sql_query('select * from segment_attribute_time_value where segment_attribute_id in (7,8) and attribute_granularity_id = 2;', con=conn2)
attr_df = pd.read_sql_query('select id, name from segment_attribute where id in (7,8);', con=conn2)
...
df_final= final_df.rename(columns={'year':'y', 'month':'m', 'week':'w', 'value_outclicks':'total_outclicks','value_average_total_outclicks':'average_total_outclicks'})

df1=pd.merge(df_final, user_df, how='left', left_on='segment_id', right_on='id')
df1 = df1[['date_index', 'y', 'm', 'w','segment_id','average_total_outclicks','total_outclicks']]
df1['average_total_outclicks'] = df1['average_total_outclicks'].astype(float)
df1.sort_values(['segment_id', 'y', 'm', 'w', 'date_index'], ascending = True, inplace = True)
sql1 = '''Select segment_id, date_index, y, m, w, average_total_outclicks, total_outclicks from audience_weekly_outclicks_frequency_trend'''
...
result_file = 'A360W_outclicks_frequency_trend.xlsx'
