...
value_df = pd.read_sql_query('select * from segment_attribute_time_value where segment_attribute_id in (1,2,3,4,5,6,9) and attribute_granularity_id = 2;', con=conn2)
...
final_df = pd.pivot_table(final_df, columns=['name'], index=['segment_id', 'date_index', 'year', 'month', 'week'], values=['value'], aggfunc=max).reset_index()
final_df.columns =['_'.join(col).rstrip('_') for col in final_df.columns.values]
df_final= final_df.rename(columns={'year':'y', 'month':'m', 'week':'w','value_sum_net_transaction_amount':'total_transaction_amount','value_avg_net_transaction_amount':'avg_transaction_amount','value_transactions_frequency':'transactions_frequency','value_sum_net_total_commission':'total_commission','value_avg_net_total_commission':'avg_total_commission','value_average_total_transaction_amount_per_customer':'average_total_transaction_amount_per_customer','value_average_total_commission_per_customer':'average_total_commission_per_customer'})

df1=pd.merge(df_final, user_df, how='left', left_on='segment_id', right_on='id')
df1 = df1[['date_index', 'y', 'm', 'w', 'segment_id','total_transaction_amount', 'avg_transaction_amount', 'transactions_frequency', 'total_commission', 'avg_total_commission', 'average_total_transaction_amount_per_customer', 'average_total_commission_per_customer' ]]
df1['average_total_commission_per_customer'] = df1['average_total_commission_per_customer'].astype(float)
df1['average_total_transaction_amount_per_customer'] = df1['average_total_transaction_amount_per_customer'].astype(float)
df1['avg_total_commission'] = df1['avg_total_commission'].astype(float)
df1['avg_transaction_amount'] = df1['avg_transaction_amount'].astype(float)
df1['total_commission'] = df1['total_commission'].astype(float)
df1['total_transaction_amount'] = df1['total_transaction_amount'].astype(float)
df1['transactions_frequency'] = df1['transactions_frequency'].astype(float)
df1.sort_values(['segment_id', 'y', 'm', 'w', 'date_index'], ascending = True, inplace = True)

sql1 = '''select segment_id, y, m, w, date_index, total_transaction_amount, average_total_transaction_amount_per_customer, transactions_frequency, average_transactions_frequency_per_customer, avg_transaction_amount, total_commission, average_total_commission_per_customer, avg_total_commission from audience_weekly_transactions_frequency_trend;'''

df2 = pd.read_sql_query(sql1, con=conn1)
df2.sort_values(['segment_id', 'y', 'm', 'w', 'date_index'], ascending = True, inplace = True)
...
result_file = 'A360W_transactions_frequency_trend.xlsx'
