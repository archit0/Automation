...
value_df = pd.read_sql_query('select * from customer_attribute_time_value where customer_attribute_id in (5) and date_index = 20190701;', con=conn2)
attr_df = pd.read_sql_query('select id, name from customer_attribute where id in (5);', con=conn2)
...
df_final= final_df.rename(columns={'customer_id':'user_id', 'year':'y', 'month':'m', 'week':'w', 'weekday':'dw','value_outclicks':'total_outclicks'})

df1=pd.merge(df_final, user_df, how='left', left_on='user_id', right_on='id')
df1 = df1[['date_index', 'y', 'm', 'w', 'dw', 'total_outclicks', 'external_id']]
df1 = df1.rename(columns={'external_id':'user_id'})
sql1 = '''Select user_id, y, m, w, dw, date_index, total_outclicks from outclicks_frequency_trend where date_index = 20190701 ;'''
...
result_file = 'outclicks_frequency_trend.xlsx'
