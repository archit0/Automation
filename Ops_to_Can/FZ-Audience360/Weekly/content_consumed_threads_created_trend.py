...
value_df = pd.read_sql_query('select * from segment_attribute_time_value where segment_attribute_id in (20,21) and attribute_granularity_id = 2;', con=conn2)
attr_df = pd.read_sql_query('select id, name from segment_attribute where id in (20,21);', con=conn2)
...
df_final= final_df.rename(columns={'year':'y', 'month':'m', 'week':'w','value_content_created_threads_posted':'total_thread_created','value_average_total_thread_created':'average_total_thread_created'})

df1=pd.merge(df_final, user_df, how='left', left_on='segment_id', right_on='id')
df1 = df1[['date_index', 'y', 'm', 'w','segment_id','total_thread_created','average_total_thread_created']]
df1['average_total_thread_created'] = df1['average_total_thread_created'].astype(float)
df1.sort_values(['segment_id', 'y', 'm', 'w', 'date_index'], ascending = True, inplace = True)
sql1 = '''Select segment_id, date_index, y, m, w, total_thread_created, average_total_thread_created from audience_weekly_content_consumed_threads_created_trend'''
...
result_file = 'A360W_content_consumed_threads_created_trend.xlsx'
