...
value_df = pd.read_sql_query('select * from segment_attribute_time_value where segment_attribute_id in (22,23,24,25) and attribute_granularity_id = 2;', con=conn2)
attr_df = pd.read_sql_query('select id, name from segment_attribute where id in (22,23,24,25);', con=conn2)
...
df_final= final_df.rename(columns={'year':'y', 'month':'m', 'week':'w','value_content_created_votes':'total_votes', 'value_content_created_ratings':'total_ratings', 'value_average_total_votes':'average_total_votes', 'value_average_total_ratings':'average_total_ratings'})

df1=pd.merge(df_final, user_df, how='left', left_on='segment_id', right_on='id')
df1 = df1[['date_index', 'y', 'm', 'w','segment_id','total_votes','total_ratings','average_total_votes','average_total_ratings']]
df1['average_total_votes'] = df1['average_total_votes'].astype(float)
df1['average_total_ratings'] = df1['average_total_ratings'].astype(float)
df1.sort_values(['segment_id', 'y', 'm', 'w', 'date_index'], ascending = True, inplace = True)
sql1 = '''Select segment_id, date_index, y, m, w, total_votes,total_ratings,average_total_votes,average_total_ratings from audience_weekly_content_consumed_votes_ratings_trend'''
...
result_file = 'A360W_content_consumed_votes_ratings_trend.xlsx'
