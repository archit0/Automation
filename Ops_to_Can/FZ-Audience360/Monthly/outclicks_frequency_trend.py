...
value_df = pd.read_sql_query('select * from segment_attribute_time_value where segment_attribute_id in (7,8) and attribute_granularity_id = 3;', con=conn2)
...
df_final= final_df.rename(columns={'year':'y', 'month':'m','value_outclicks':'total_outclicks','value_average_total_outclicks':'average_total_outclicks'})

df1=pd.merge(df_final, user_df, how='left', left_on='segment_id', right_on='id')
df1 = df1[['date_index', 'y', 'm', 'segment_id','average_total_outclicks','total_outclicks']]
...
sql1 = '''Select segment_id, date_index, y, m, average_total_outclicks, total_outclicks from audience_monthly_outclicks_frequency_trend'''
...
result_file = 'A360M_outclicks_frequency_trend.xlsx'
