...
value_df = pd.read_sql_query('select * from segment_attribute_time_value where segment_attribute_id in (28) and attribute_granularity_id = 2;', con=conn2)
attr_df = pd.read_sql_query('select id, name from segment_attribute where id in (28);', con=conn2)
...
df1=pd.merge(final_df, user_df, how='left', left_on='segment_id', right_on='id')

_df = df1['value_threads_viewed'].apply(lambda x: json.loads(x))
__df = pd.DataFrame(_df.to_list())
df1 = pd.merge(df1, __df, right_index=True, left_index=True)
df1 = df1.rename(columns={'year':'y', 'month':'m', 'week':'w'})

df1 = df1[['date_index', 'y', 'm', 'w', 'segment_id','hot_deal_views','fire_deal_views','front_page_views']]
df1.sort_values(['segment_id', 'y', 'm', 'w', 'date_index'], ascending = True, inplace = True)

sql1 = '''select segment_id, y, m, w, date_index, hot_deal_views, fire_deal_views, front_page_views from audience_weekly_thread_popularity_category;'''
...
result_file = 'A360D_thread_popularity_category.xlsx'
