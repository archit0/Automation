...
value_df = pd.read_sql_query('select * from segment_attribute_time_value where segment_attribute_id in (28) and attribute_granularity_id = 1;', con=conn2)
...
df1 = df1.rename(columns={'year':'y', 'month':'m', 'week':'w', 'weekday':'dw'})

df1 = df1[['date_index', 'y', 'm', 'w', 'dw', 'segment_id','hot_deal_views','fire_deal_views','front_page_views']]
df1.sort_values(['segment_id', 'y', 'm', 'w', 'dw', 'date_index'], ascending = True, inplace = True)

sql1 = '''select segment_id, y, m, w, dw, date_index, hot_deal_views, fire_deal_views, front_page_views from audience_thread_popularity_category;'''
