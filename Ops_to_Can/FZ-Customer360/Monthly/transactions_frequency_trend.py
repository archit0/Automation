...
user_df = pd.read_sql_query('select id, external_id from customer', con=conn2)
value_df = pd.read_sql_query('select * from customer_attribute_time_value where customer_attribute_id in (7,25) and date_index = 20190701;', con=conn2)
attr_df = pd.read_sql_query('select id, name from customer_attribute where id in (7,25);', con=conn2)

final_df = pd.merge(attr_df, value_df, how='left', left_on='id', right_on='customer_attribute_id')
final_df = pd.pivot_table(final_df, columns=['name'], index=['customer_id', 'date_index', 'year', 'month', 'week','weekday'], values=['value'], aggfunc=max).reset_index()
final_df.columns =['_'.join(col).rstrip('_') for col in final_df.columns.values]
df_final= final_df.rename(columns={'customer_id':'user_id', 'year':'y', 'month':'m', 'week':'w', 'weekday':'dw','value_sum_net_transaction_amount':'total_transaction_amount','value_avg_net_transaction_amount':'avg_transaction_amount','value_transactions_frequency':'transactions_frequency','value_sum_net_total_commission':'total_commission','value_avg_net_total_commission':'avg_total_commission'})

df1=pd.merge(df_final, user_df, how='left', left_on='user_id', right_on='id')
df1 = df1[['date_index', 'y', 'm', 'w', 'dw', 'total_transaction_amount','avg_transaction_amount','transactions_frequency','total_commission','avg_total_commission', 'external_id']]
df1 = df1.rename(columns={'external_id':'user_id'})

sql1 = '''Select user_id, y, m, w, dw, date_index,total_transaction_amount,avg_transaction_amount,transactions_frequency,total_commission,avg_total_commission  from transactions_frequency_trend where date_index = 20190701 ;'''

df2 = pd.read_sql_query(sql1, con=conn1)
df2.sort_values(['user_id', 'y', 'm', 'w', 'dw', 'date_index'], ascending = True, inplace = True)
print ('Retrieved featurized query through SQL...')

print("comperision started")
unique = 'user_id'  # storing unique column name in variable as index for comparison of test result later...
...
result_file = 'transactions_frequency_trend.xlsx'
