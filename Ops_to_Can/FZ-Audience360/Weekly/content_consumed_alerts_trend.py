...
value_df = pd.read_sql_query('select * from segment_attribute_time_value where segment_attribute_id in (18,19) and attribute_granularity_id = 2;', con=conn2)
...
df_final= final_df.rename(columns={'year':'y', 'month':'m', 'week':'w','value_content_consumed_number_of_subscriptions':'total_number_of_alerts', 'value_average_alerts_per_user':'average_alerts_per_user'})

df1=pd.merge(df_final, user_df, how='left', left_on='segment_id', right_on='id')
df1 = df1[['date_index', 'y', 'm', 'w', 'total_number_of_alerts', 'average_alerts_per_user', 'segment_id']]
...
sql1 = '''SELECT date_index, y, m, w, total_number_of_alerts, average_alerts_per_user, segment_id from audience_weekly_content_consumed_alerts_trend;'''
...
result_file = 'A360W_content_consumed_alerts_trend.xlsx'
